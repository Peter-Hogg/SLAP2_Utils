...
def plotPixTrace(ax, dffData, hz, start, end):#, y_min, y_max):
    ax.clear()
    ...
    """    for k in range(len(ax.lines)):
        if k >= dffData.shape[0]:
            ax.lines[k].set_data([], [])
        else:
            data_slice = dffData[k, start:end]
            if data_slice.size > 0:
                ax.lines[k].set_data(time, data_slice + (.15 * k))
                y_min_slice = (data_slice + (.15 * k)).min()
                y_max_slice = (data_slice + (.15 * k)).max()
...
class PlotCanvas(FigureCanvas):
    ...
    def plot(self, dffData, hz, start, end):
        if not self.initialized:
            self.initialize_plot(dffData, hz, start, end)
        else:
            plotPixTrace(self.ax, dffData, hz, start, end)

    def initialize_plot(self, dffData, hz, start, end):
        print(dffData.shape)
        time = np.arange(0, int(end - start) / hz, 1 / hz) + (start / hz)
        self.ax.clear()
        y_min, y_max = -0.05, 1
    ...
    def update_plot(self, dffData, hz, ix, window_size):
        plotPixTrace(self.ax, dffData, hz, ix, ix + window_size)

class superPixelPlot(QMainWindow):
    def __init__(self, roi_index, channel, hdatafile):
        super().__init__()
        ...
        self.canvas.plot(dffTraces, hz, 0, self.window_size)
    ...
    def update_frame(self, ix):
        self.canvas.update_plot(self.dffTraces, self.hz, ix, self.window_size)

if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = superPixelPlot(roi_index=0, channel=1, hdatafile=None)  # Replace with actual data
    window.show()
    sys.exit(app.exec())
