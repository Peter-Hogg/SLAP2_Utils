function [startTime, sType] = SingleBar(win, angle, speed, length, width, internalRotation, daq)
...
barStartX = initialX - deltaX;  % Position outside the screen in the opposite direction
barStartY = initialY - deltaY;
...
while (barPosition(1) >= -barWidth && barPosition(1) <= windowRect(3)) || (barPosition(1) <= -barWidth && barPosition(2) >= -barHeight && barPosition(2) <= windowRect(4)) || (barPosition(1) >= windowRect(3) && barPosition(2) >= -barHeight && barPosition(2) <= windowRect(4))
