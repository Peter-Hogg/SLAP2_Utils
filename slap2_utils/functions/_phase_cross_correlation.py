...
def _upsampled_dft(data, upsampled_region_size, upsample_factor=1, axis_offsets=None):
    ...
    ----------
    data : array
        The icput data array (DFT of original data) to upsample.
    ...
    if axis_offsets is None:
        ...
        if len(axis_offsets) != data.ndim:
            raise ValueError(
                "number of axis offsets must be equal to icput "
...
def _compute_error(cross_correlation_max, src_amp, target_amp):
    ...
    #with cupyx.errstate(invalid="ignore"):
...
def _disambiguate_shift(reference_image, moving_image, shift):
    ...
    for test_slice in itertools.product(*splits_per_dim):
        ...
        if reference_tile.size > 2:
            # In the case of zero std, cp.corrcoef returns NaN and warns
            # about division by zero. This is expected and handled below.
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", category=RuntimeWarning)
                corr = cp.corrcoef(reference_tile, moving_tile)[0, 1]
        if corr > max_corr:
            max_corr = corr
    ...
    return cp.array(real_shift_acc)
...
def phase_cross_correlation(
    ...
    *,
    ...
    space : string, one of "real" or "fourier", optional
        Defines how the algorithm interprets icput data. "real" means
        data will be FFT'd to compute the correlation, while "fourier"
        data will bypass FFT of icput data. Case insensitive. Not
    ...
    shift : ndarray
        ...
        the axis order of the icput array.
    error : float
    ...
    reference_image = cp.array(reference_image)
    moving_image = cp.array(moving_image)
    ...
    if cp.isnan(CCmax) or cp.isnan(src_amp) or cp.isnan(target_amp):
        raise ValueError(
            ...
            "icput data or use the `reference_mask`/`moving_mask` "
            "keywords, eg: "
